...
ADD_METRIC(21, zesDeviceEnumPsus, zes_psu_handle_t, zesPsuGetProperties,
...
ADD_METRIC(24, zesDeviceEnumFans, zes_fan_handle_t, zesFanGetProperties,
...
ze_result_t zesFanGetState(zes_fan_handle_t handle, zes_fan_speed_units_t units,
                           int32_t *speed) {
  ze_result_t ret = metric_args_check(26, "zesFanGetState", handle, speed);
  if (ret == ZE_RESULT_SUCCESS) {
...
ze_result_t zesFanGetConfig(zes_fan_handle_t handle, zes_fan_config_t *config) {
  ze_result_t ret = metric_args_check(27, "zesFanGetConfig", handle, config);
  if (ret == ZE_RESULT_SUCCESS) {
